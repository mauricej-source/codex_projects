...
const scoreRelevance = (job, profile) => {
...
export async function searchFallbackJobs(profile, warning) {
    const preferredLocations = profile.preferences.preferredLocations.map((location) => location.toLowerCase());
    const jobs = [...mockJobs]
        .filter((job) => {
