const defaultWorkplaceTypes = ["Remote", "Hybrid", "On-site"];
        ...
        preferences: {
            ...
            preferredLocations: Array.isArray(input.preferences?.preferredLocations)
                ...
                : input.location
                    ? [input.location]
                    : [],
            minimumSalary: typeof input.preferences?.minimumSalary === "number" ? input.preferences.minimumSalary : undefined
        },
        diceSearch: {
            location: input.diceSearch?.location ?? input.location ?? "",
            workplaceTypes: Array.isArray(input.diceSearch?.workplaceTypes) && input.diceSearch.workplaceTypes.length > 0
